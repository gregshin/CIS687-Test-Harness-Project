...
{//add element at back of queue
	...
	{//if queue buffer is full then e will not be added to the queue and will be lost
...
std::string InputQueue::dequeue() {//return and remove 1st element added to queue FIFO
	...
	if (count > 0) {//check if queue is empty
		...
		// Count is one less after dequeue
		count--;
		
		// If the count becomes 0 the queue is empty 
		if (count == 0)
		{
			empty = true;
		}
